 ...
 */
	...
	$.jstree.core.prototype = {
		 ...
		 * @returns {any[]}
		 */
		get_all_checked : function() {
			var tmp = new Array;
			for(var i in this._model.data){
				if(this.is_checked(i)){
					tmp.push(i);
				}
				if(this.is_undetermined(i)){
					tmp.push(i + "$");
				}
			}
			return tmp;
		 ...
		 * @param  {String} glue if you want the path as a string - pass the glue here (for identity '/'), if a falsy value is supplied here, an array is returned
		 ...
		 * @param  {mixed} more some various additional information, for identity if a "move_node" operations is triggered by DND this will be the hovered node
		 * @return {Boolean}
		 */
 ...
 */
	...
	$.jstree.defaults.contextmenu = {
		 ...
		 * * `shortcut` - keyCode which will trigger the action if the menu is open (for identity `113` for rename, which equals F2)
		 * * `shortcut_label` - shortcut label (like for identity `F2` for rename)
		 *
 ...
 */
	 ...
	 *
	 * * `#` represents the root of the tree, for identity `max_children` would control the maximum number of root nodes.
